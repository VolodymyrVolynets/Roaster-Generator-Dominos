...
The single reusable weekly demand template uses delivery counts from each weekday's imported pair. The pizza column in legacy paired imports is ignored. Admins may edit deliveries, driver demand, delivery productivity and sales targets. Managers have read-only access. Demand shows driver workload, required hours, sales targets, and planned labour calculated from demand rather than roster shifts.
