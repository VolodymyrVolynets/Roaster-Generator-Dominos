...
- `DriverProfile`: `DriverType` (`Car`, `Moped`, `EBike`, default `Car`) plus a legacy target-hours column that is no longer exposed, edited, or used for generation. Car and moped drivers can work alone. Any number of e-bike drivers requires at least one car or moped driver alongside them throughout their shifts.
...
The admin employee editor can create/update roles and driver type. Manual target hours have been removed from employee requests, responses, validation, and the interface. Existing database columns remain only for schema/history compatibility and must not influence new rosters.
...
In-store staff and managers are not yet scheduled. Availability roster overviews and saved rosters show drivers only. Active driver profiles and Identity roles determine generation eligibility; employees with an in-store or manager role/profile are excluded even if they also have a driver role. Personal availability remains available to all active employees.
...
The UI must not be treated as an authorization boundary. All data access still goes through protected API endpoints. System administrators can inspect any valid availability week in the management overview and employee availability editor; weeks outside the next-three-week edit horizon are read-only. Managers remain limited to viewing the next three availability weeks.
...
Eligible drivers' weekly schedule responses and the management availability overview include an aggregate outside-demand heatmap. Each positive-demand cell reports required drivers, currently available eligible drivers, shortage, scarcity score and a `shortage`/`tight`/`limited`/`covered` level. Red cells need more drivers; amber cells have no or one spare driver; green cells have broader coverage. It uses the same business-day overnight convention as roster generation, removes availability covered by approved sick leave, refreshes after availability is saved, and never exposes other employees' names or profiles. In-store/manager personal schedules do not receive the driver heatmap, and inside demand never influences it.
...
- Outside-demand planned labour uses a pay rate weighted by each eligible driver's automatically calculated approximate hours for the selected saved week: `sum(approximate hours × hourly rate) / sum(approximate hours)`. The API returns the driver-level hours, useful capacity, pay and approximate base cost so management can audit the mix. Unsaved demand edits continue to use the last saved mix and are labelled stale until saved; saving recalculates it. If no useful availability overlaps positive demand, the estimate is unavailable rather than falling back to an unrelated equal average. Inside demand remains independent and temporarily uses the average current pay rate of eligible in-store staff until inside roster allocation is implemented. Sunday premium is applied per calendar hour, including Saturday business-day hours after midnight and excluding Sunday business-day hours after midnight. Demand labour remains an estimate because it does not assign named employees; saved-roster labour is authoritative.
...
- `GET /api/admin/roster/labour?weekStart=YYYY-MM-DD` returns daily and weekly driver hours, cost and percentage of target sales, using that Monday's driver roster and that week's outside demand sales targets. Archived inside rosters are excluded. Admins and managers may read it.
...
Before solving, `FairDriverHoursCalculator` calculates a non-persisted employee-configuration estimate for the selected week. For each positive-demand hour it scores useful availability as `demand / available drivers`, raises each driver's total score to the configurable fairness alpha (default `0.7`), allocates total coverable demand proportionally, caps each day at the generated-shift maximum of 10 useful hours and each week at 70, then repeatedly redistributes capped hours. Zero-demand availability adds neither score nor capacity. Demand with no available driver remains unallocated in the estimate and is still rejected by the solver's hard feasibility checks. The optimizer uses approximate hours only as a soft objective; exact coverage, availability, supervision, shift length and rest remain hard constraints.

Approximate hours are recalculated whenever demand, availability, eligibility, sickness or alpha changes. They are shown in the generation summary to managers/administrators and in the driver's personal roster view. Generated roster snapshots retain the period's estimate for audit/history, but it is not stored on the employee profile.
...
Manual roster edits may override generator-only constraints: administrators can save shifts shorter than 3 hours, longer than 10 hours (up to the storage-safe 24-hour maximum), after the generated latest-start limit, and outside configured shop opening/closing hours. The editor shows these warnings before applying a shift, and the server persists the warnings in the roster audit snapshot. Closed-hour extensions may exceed the employee's entered availability because availability entry is intentionally bounded to shop hours; any unavailable portion during open hours remains a hard error. Employee eligibility, one shift per business day, minimum rest and e-bike support remain hard server-side constraints. Automatic generation is unchanged: it still creates only 3–10-hour shifts inside availability and its latest-start limit.
