...
Before solving, `FairDriverHoursCalculator` calculates a non-persisted employee-configuration estimate for the selected week and roster kind. For each positive-demand hour it scores useful availability as `demand / available eligible employees`, raises each employee's total score to the configurable fairness alpha (default `0.7`), allocates total coverable demand proportionally, caps each day at the generated-shift maximum of 10 useful hours and each week at 70, then repeatedly redistributes capped hours. Zero-demand availability adds neither score nor capacity. Demand with no available employee remains unallocated in the estimate and is still rejected by the solver's hard feasibility checks. The optimizer uses approximate hours only as a soft objective; exact coverage, availability, supervision, shift length and rest remain hard constraints. `ApproximateHoursWeight` controls that soft objective; its deployed database column retains its legacy name to avoid a data migration.

Approximate hours are recalculated whenever demand, availability, eligibility, sickness or alpha changes. They are shown in the generation summary to managers/administrators and in the driver's personal roster view. New roster snapshots retain the exact fractional estimate and approximate-hours percentages for audit/history, but it is not stored on the employee profile. Historical fairness reads that exact snapshot estimate and never falls back to a profile target.
